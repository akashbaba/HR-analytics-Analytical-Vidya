...
anova(hrlogistic,test='Chisq')
library(caret)
logisticpredict=ifelse(hrlogisticpredict>=0.50,1,0)
head(logisticpredict)
logisticpredict$logisticpredict=as.factor(logisticpredict$logisticpredict)
confusionMatrix(hrtraindf$is_promoted,
...
trCtrl=trainControl(method='cv',
                    number=2)
tree=expand.grid(n.trees=c(1000),
                 interaction.depth=c(2),
                 shrinkage=0.1,
                 n.minobsinnode = 10)
hrgbm=train(is_promoted~.,data=hrtraindf[-1],
            method='gbm',
            trControl=trCtrl,
            verbose=T,
            tuneGrid=tree)
